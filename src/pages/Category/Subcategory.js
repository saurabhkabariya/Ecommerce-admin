...
const Subcategory = () => {
  ...
  useEffect(() => {
    ...
    fetchdatafromapi("/api/category").then((res) => {
      setsubcatdata(res);
      setsubcategorylist(res.subcatlist);
      setcatdata(res);
    });
        ...
        <div className="productlistbox w-100 my-3 shadow d-flex res-col">
          ...
          <div className="showpath" role="presentation">
            <Breadcrumbs aria-label="breadcrumb">
              ...
              <Typography sx={{ color: "text.primary" }}>Sub Categories</Typography>
            </Breadcrumbs>
          </div>
          <Link to={'/subcategory/add'} className="mr-3"><Button className="btn-blue pl-4 pr-4">Add Sub Category</Button></Link>
                        ...
                        <tr className="productcard" key={index}>
                          ...
                          <td>
                            {item?.children?.length!==0 && item?.children?.map((subcat,index)=>{
                              return(
                                <span className="badge badge-primary mx-1" key={index}>{subcat?.name}<IoCloseSharp className="cursor" onClick={()=>deletesubcat(subcat?._id)}/></span>
                              )
                            })}
                          </td>
                          {/* <td>{item.subcat}</td> */}
                          ...
                          <td>
                            <div className="actions d-flex align-items-center">
                              ...
                              <Link to={`/subcategory/edit/${item?._id}`}><Button className="success" color="success">
                                <MdEdit />
                              </Button>
                              </Link>
